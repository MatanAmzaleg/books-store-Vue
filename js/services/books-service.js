...
function prepareData(book) {
  const { id, volumeInfo: { publishedDate = 1999, categories, authors = 'puki', description = 'loremloremlorem loremloremloreml oremloremloremlore mloremloremloremloremlorem loremloremloremlorem', language, imageLinks: { thumbnail } = 'img/notFound.jpg', pageCount, title } } = book
  const newBook = { id, categories, title, authors, publishedDate, subtitle: 'subtitlesubtitlesubtitle', description, language, reviews: [], thumbnail, listPrice: { amount: 150, currencyCode: 'EUR', isOnSale: true }, pageCount }
  return newBook
}
...
function getNextBookId(bookId) {
  return storageService.query(BOOKS_KEY)
    .then(books => {
      ...
      return {
        nextId: (idx === books.length - 1) ? books[0].id : books[idx + 1].id,
        prevId: (idx === 0) ? books[books.length - 1].id : books[idx - 1].id
      }
    })
}
...
function putReview(review, bookId) {
...
function addGoogleBook(book) {
  ...
  const newBook = prepareData(book)
